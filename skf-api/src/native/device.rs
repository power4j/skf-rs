...
use crate::native::types::{DeviceInfo, BOOL, BYTE, DEV_HANDLE, LPSTR, ULONG};
...
extern "C" {
    ...
    ///
    /// [handle]		`[OUT]`返回设备操作句柄
    pub fn SKF_ConnectDev(sz_name: LPSTR, handle: *mut DEV_HANDLE) -> ULONG;

    /// 断开一个已经连接的设备，并释放句柄。
    ///
    /// [handle]		`[IN]`连接设备时返回的设备句柄
    pub fn SKF_DisConnectDev(handle: DEV_HANDLE) -> ULONG;
    ...
    pub fn SKF_SetLabel(handle: DEV_HANDLE, sz_label: LPSTR) -> ULONG;
    ...
    ///
    /// [dev_info]	[OUT]返回设备信息
    pub fn SKF_GetDevInfo(handle: DEV_HANDLE, dev_info: *mut DeviceInfo) -> ULONG;
    ...
    pub fn SKF_LockDev(handle: DEV_HANDLE, timeout: ULONG) -> ULONG;

    /// 释放对设备的独占使用权
    ///
    /// [handle]		`[IN]`连接设备时返回的设备句柄
    pub fn SKF_UnlockDev(handle: DEV_HANDLE) -> ULONG;
